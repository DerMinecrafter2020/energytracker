...
// 1. Import Globe, fetchTranslations, saveTranslations
...
const stateToAdd = 
  ...
  const handleSaveTranslations = async () => {
    try {
      ...
      setMsg('Übersetzungen erfolgreich gespeichert');
    } catch(e) {
      setError('Ungültiges JSON oder Speicherfehler: ' + e.message);
    }
  };
;
// find const [activeTab, setActiveTab] = useState('overview'); and inject after
...
// Find end of tabs (before final closing div)
content = content.replace("{activeTab === 'settings' && (", viewToAdd + "\n        {activeTab === 'settings' && (");
